...
public class CanecaService {
    ...
    /**
     * Recupera todas as canecas do cliente, verifica se a caneca de fato pertence
     * ao cliente, se pertencer, deleta e retorna true, senao, retorna false Nota:
     * Este método também exlui todas imagens da caneca
     * 
     * @param id-      id da caneca
     * @param clientId - id do cliente para verificar se a caneca pertence ao
     *                 cliente
     */
    public boolean deleteById(Long id) {
	Caneca c = canecaRepository.findById(id);
	imgRepository.findAll(c.getId()).forEach(x -> imageService.deleteById(x.getId()));
